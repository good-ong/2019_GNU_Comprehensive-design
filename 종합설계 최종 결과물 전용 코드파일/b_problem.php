...
<div class="cate_center">
  ...
  <div id="right_content">
    <?php
    include "db.php";
    $result = mq('select count(*) as cnt from b_problem');
    ...
    <div style="padding-bottom:50px;">
      <table class="list-table" width="850" style="text-align:center;">
        ...
        $sql = mq("select * from b_problem order by problem_idx desc limit ".$page_list.", 10");
        while($board = $sql->fetch_array()) {
          $title=$board["problem_title"];
          if(strlen($title)>30) {
            $title=str_replace($board["problem_title"],mb_substr($board["problem_title"],0,30,"utf-8")."...",$board["problem_title"]); //title이 30을 넘어서면 ...표시
          }
        ?>
        <tbody>
          <tr>
            <td width="70"><a href="b_problem_view.php?idx=<?php echo $board["problem_idx"];?>"><?php echo $board['problem_idx']; ?></a></td>
            <td width="500"><a href="b_problem_view.php?idx=<?php echo $board["problem_idx"];?>"><?php echo $title;?></a></td>
            <td width="120"><?php echo $board['user_id']?></td>
            <td width="100"><?php echo $board['problem_date']?></td>
            <td width="100"><?php echo $board['problem_hit']; ?></td>
          </tr>
        </tbody>
        <?php } ?>
      </table>
      <div id="write_btn">
        ...
        <table>
          <tr>
          <?php if ($_GET['page'] > 1) {?>
            <td width="125" style="text-align:right;"><a href="b_problem.php?page=<?php echo $list['prev']; ?>">prev</a></td>
          <?php } else { ?>
          ...
          <?php } ?>
            <td width="600" style="text-align:center;">
              ...
              {
                echo "<a href='b_problem.php?page=".$w."'>".$w."</a> ";
              }
              ?>
            </td>
          <?php if ($_GET['page'] < $list['total_page']) {?>
            <td width="125" style="text-align:left;"><a href="b_problem.php?page=<?php echo $list['next']; ?>">next</a></td>
          <?php } else { ?>
        ...
        <table>
          <tr>
            ...
            <td width="500" style="text-align:right;">
              <?php if ($user_id){?>
              <a href="problem_write.php"><button>글쓰기</button></a>
              <?php } ?>
            </td>
          </tr>
        </table>
